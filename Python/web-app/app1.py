...
def histogram_to_base64(image):
    ...
    """
    # Convert image to RGB and extract channels
    ...
    # Average the histograms to get a mean histogram
    mean_hist = [(r_hist[i] + g_hist[i] + b_hist[i]) / 3 for i in range(256)]

    # Plot the histogram
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar(range(256), mean_hist, color='blue', alpha=0.6)
