...
red_spec = df.iloc[:, 0].tolist()
blue_spec = df.iloc[:, 1].tolist()
green_spec = df.iloc[:, 2].tolist()

print(len(red_spec))
print(len(blue_spec))
print(len(green_spec))
...
cv2.imwrite('./Assignment3/Histogram_Specified_Moon.jpg', specified_image_bgr)
